...
output_path = '../clustering_data/mean_shift_' + str(distance_type) + '.txt'
...
linkage_matrix = linkage(distances, method='complete')  # You can choose the linkage method
cluster_assignments = fcluster(linkage_matrix, t=1, criterion='distance')  # Adjust the threshold 't' based on your data


print(len(cluster_assignments))

unique_labels = np.unique(cluster_assignments[cluster_assignments != -1])
num_groups = len(unique_labels)
print(num_groups)
# Print the results
for i in range (0, num_groups + 5):
...
for g1 in range(0, num_groups + 1):
    min_dist.append(list())
    avg_dist.append(list())
    if g1 == 0:
        continue
    for g2 in range(0, num_groups + 1):
        avg_dist[g1].append(0)
        min_dist[g1].append(100)
    for g2 in range(g1, num_groups + 1):
